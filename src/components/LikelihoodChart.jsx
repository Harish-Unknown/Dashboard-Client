...
const LikelihoodChart = ({ likelihoodData }) => {
    ...
    // Shuffle function to shuffle colors randomly
    const shuffle = array => {
    ...
    // Process likelihood data based on selected filters
    ...
    // Define chartData and other necessary data and functions
    const uniqueDates = new Set(validData.map(entry => {
        const date = new Date(entry.published);
        return `${date.toLocaleString('default', { month: 'short' })}-${date.getFullYear()}`;
    }));

    const years = Array.from(uniqueDates).map(date => date.split('-')[1]); // Extract years

    const shuffledColors = shuffle([...manualColors]); // Shuffle the colors randomly

    const colorMap = {}; // Map years to colors
